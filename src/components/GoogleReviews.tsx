...
export const GoogleReviews = () => {
  ...
  const combineWithFallback = (liveReviews: Review[]): Review[] => {
    ...
    const existingTexts = new Set(liveReviews.map(r => r.text.trim().toLowerCase()));
    const existingAuthors = new Set(liveReviews.map(r => r.authorName.trim().toLowerCase()));

    const supplements: Review[] = [];
    for (const fallback of fallbackReviewsData) {
      if (liveReviews.length + supplements.length >= 6) break;
      if (!existingTexts.has(fallback.text.trim().toLowerCase()) && !existingAuthors.has(fallback.authorName.trim().toLowerCase())) {
  ...
  return (
    <section id="reviews" className="py-24 px-8 bg-slate-50/70 border-y border-slate-200/80 relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Button to View More Testimonials on Google Business Page */}
        <div className="mt-12 flex justify-center">
          <a
            href={googleMapsUrl}
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-3 bg-white hover:bg-slate-50 text-navy-900 border-2 border-slate-200 hover:border-yellow-500 px-8 py-4 rounded-full font-bold text-sm shadow-md hover:shadow-xl transition-all group cursor-pointer"
          >
            ...
            <span>More Testimonials ({userRatingCount}+ Reviews on Google)</span>
            <ExternalLink size={16} className="text-navy-400 group-hover:text-yellow-600 transition-colors" />
          </a>
        </div>
